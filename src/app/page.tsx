...
const features = [
  {
    ...
    description: 'State-of-the-art classrooms and laboratories for collaborative learning.',
  ...
  },
  {
    ...
    description: 'Passionate educators with real-world experience and innovative teaching methods.',
  ...
  },
  {
    ...
    description: 'Individualized attention and customized learning paths for every student.',
  ...
  },
  {
    ...
    description: 'International programs and cultural exchanges for global success.',
...
const coreValues = [
  {
    ...
    description: "Rigorous curriculum that challenges students to reach their full potential through modern educational approaches.",
    icon: (
  ...
  },
  {
    ...
    description: "Developing strong moral character, leadership skills, and ethical values in responsible global citizens.",
    icon: (
      <svg className="w-10 h-10 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  },
  {
    ...
    description: "Encouraging innovative thinking through modern teaching methods and hands-on learning experiences.",
    icon: (
      <svg className="w-10 h-10 md:w-12 md:h-12" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function Home() {
  return (
    <div className="min-h-screen bg-primary-50">
      ...
      {/* Stats Section */}
      <section className="py-12 md:py-24 bg-primary-50 relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
            {stats.map((stat, index) => (
              <motion.div
                ...
                className="bg-white/50 backdrop-blur-sm rounded-2xl p-4 md:p-8 shadow-soft-lg hover:shadow-soft-xl transition-all duration-500 text-center group hover:-translate-y-1"
              >
                <div className="font-bold text-2xl sm:text-3xl md:text-4xl lg:text-5xl mb-1 md:mb-2 text-[#0A192F]">
                  {stat.value}
                </div>
                <div className="text-sm sm:text-base md:text-lg font-semibold text-gray-800 mb-1">{stat.label}</div>
                <div className="text-xs sm:text-sm md:text-base text-gray-600">{stat.description}</div>
              </motion.div>
            ))}
          </div>
        </div>
            ...
            {galleryImages.map((image, index) => (
              ...
              >
                ...
                <div className="absolute bottom-0 left-0 right-0 p-4 text-white">
                  <p className="text-base font-medium text-left">{image.alt}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 md:gap-8">
            {coreValues.map((value, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.2 }}
                className="bg-[#0A192F] backdrop-blur-sm rounded-2xl p-6 md:p-8 shadow-[0_4px_20px_rgba(0,0,0,0.2)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.3)] transition-all duration-500 group hover:-translate-y-1 border border-[#1E3A8A]/30"
              >
                <div className="text-white mb-4 md:mb-6 transform group-hover:scale-110 transition-all duration-500">
                  {value.icon}
                </div>
                <h3 className="text-xl md:text-2xl font-bold text-white mb-3 md:mb-4">{value.title}</h3>
                <p className="text-sm md:text-lg text-gray-300 leading-relaxed">{value.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-12 md:py-24 bg-primary-50 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-6 animate-fade-in">
            Ready to Join Our Community?
          </h2>
          <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-gray-600 mb-8 md:mb-12 max-w-3xl mx-auto font-medium animate-slide-up">
            Take the first step towards a brighter future. Apply now to become part of our vibrant academic community.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 animate-slide-up" style={{ animationDelay: '200ms' }}>
            <Button 
              variant="primary" 
              size="lg"
              className="bg-[#0A192F] hover:bg-[#112240] text-white shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 w-full sm:w-auto sm:min-w-[200px]"
            >
              <Link href="/admissions" className="font-semibold px-6 sm:px-8">
                Start Application
              </Link>
            </Button>
            <Button 
              variant="outline" 
              size="lg"
              className="border-2 border-[#0A192F] text-[#0A192F] hover:bg-[#0A192F] hover:text-white shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-300 w-full sm:w-auto sm:min-w-[200px]"
            >
              <Link href="/contact" className="font-semibold px-6 sm:px-8">
                Contact Us
              </Link>
            </Button>
          </div>
